    ...
    ChatRequest {
        ...
        tools: vec![Tool {
            ...
            function: ToolFn {
                ...
                parameters: json::from_str(r#"{"type":"object","properties":{"keys":{"type":"array","items":{"type":"string"}}},"required":["keys"]}"#).unwrap(),
            },
        }],
    }
}
...
fn main() {
    ...
    if mode == "--sse" {
        // live incremental-streaming proof over rustls: drip endpoint sends bytes gradually
    ...
    // correctness pass over every sample (also warms up)
    let mut content_len = 0usize;
    for s in OPENAI_SAMPLES {
        let c: Chunk = json::from_str(s).expect("openai");
        content_len += c.choices[0].delta.content.as_deref().map_or(0, str::len);
    }
    for s in ANTHROPIC_SAMPLES {
        let e: AEvent = json::from_str(s).expect("anthropic");
    ...
    // parse throughput: 200k iterations over the realistic sample mix
    ...
    for i in 0..N {
        ...
        bytes += s.len();
        let c: Chunk = json::from_str(s).unwrap();
    ...
    for i in 0..N {
        ...
        bytes += s.len();
        let e: AEvent = json::from_str(s).unwrap();
    ...
    // serialize throughput: 100k full request bodies
    const M: usize = 100_000;
    let req = make_request();
    let mut out_bytes = 0usize;
    let t = Instant::now();
    for _ in 0..M {
        out_bytes += json::to_string(&req).len();
    }
    let dt = t.elapsed();
